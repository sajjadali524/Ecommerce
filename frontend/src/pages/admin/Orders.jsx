...
const Orders = () => {
  ...
  var date = null;
  allOrders.map((item) => {
    const format = item.createdAt;
    date = new Date(format).toLocaleDateString();
  });
    ...
    const fetchAllOrders = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8000/api/v1/order/get-all-order",
          {
            headers: {
    ...
    <div className="lg:pt-5 pt-0 space-y-2 overflow-x-auto w-full">
      ...
      {allOrders.map((order) => {
        return (
          <div
            key={order._id}
            className="grid lg:grid-cols-2 grid-cols-1 border p-5 overflow-x-auto w-full lg:space-y-0 space-y-5"
          >
            ...
            <div className="flex justify-between w-full">
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="text-[14px]">Date: {date}</p>
                </div>
              </div>
              ...
              <div>
                <select
                  ...
                  onChange={(e) => updatePaymentStatus(e, order._id)}
                >
